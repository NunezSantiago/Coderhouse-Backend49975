...
export class productsController{
    ...
    static async createProduct(req, res){
        ...
        if(!title || !description || !code || !price || !stock || !category){
            ...
            //customError.customError("Not enough information", "Product information is not complete, please, make sure to enter all necessary information", STATUS_CODES.ERROR_ARGUMENTOS, INTERNAL_CODES.ARGUMENTS, productErrors.productArgsError({title, description, code, price, stock, category, status, thumbnails}))
            // req.logger.error(
            //     customError.customError("Not enough information", "Product information is not complete, please, make sure to enter all necessary information", STATUS_CODES.ERROR_ARGUMENTOS, INTERNAL_CODES.ARGUMENTS, productErrors.productArgsError({title, description, code, price, stock, category, status, thumbnails}))
            // )
        ...
        if(exist){
            ...
            req.logger.error(customError.customError("Code already exists", `Product with code: ${code} is already registered`, STATUS_CODES.ERROR_DATOS_ENVIADOS, INTERNAL_CODES.ARGUMENTS, productErrors.productCodeError(code)))
        }

        if(isNaN(stock)){
            req.logger.error(customError.customError("Stock is not a number", "Stock value provided must be a numeric value", STATUS_CODES.ERROR_DATOS_ENVIADOS, INTERNAL_CODES.ARGUMENTS, productErrors.stockNaN(stock)))
        }
        
        if(isNaN(price)){
            req.logger.error(customError.customError("Price is not a number", "Price value provided must be a numeric value", STATUS_CODES.ERROR_DATOS_ENVIADOS, INTERNAL_CODES.ARGUMENTS, productErrors.priceNaN(price)))
        }
        ...
        if(newProduct.error){
            //req.logger.fatal(newProduct.error)
            ...
            //req.logger.error(customError.customError("Database unexpected error", "Database unexpected error, please, retry later", STATUS_CODES.ERROR_ARGUMENTOS, INTERNAL_CODES.DATABASE, newProduct.error.message))
        } else {
